    ...
    pub fn new(
        ...
        appsrc.set_callbacks(
            gst_app::AppSrcCallbacks::builder()
                ...
                .enough_data(move |_| {
                    log::debug!("AppSrc enough data for stream {stream_id}");
                })
                .build(),
        );
...
impl GStreamerBackend {
    ...
    pub fn set_format(&self, params: &PcmParams) -> Result<AudioFormat> {
        let format = match params.format {
            ...
            VIRTIO_SND_PCM_FMT_S8 => AudioFormat::S8,
            VIRTIO_SND_PCM_FMT_U8 => AudioFormat::U8,
            VIRTIO_SND_PCM_FMT_S16 => AudioFormat::S16be,
            VIRTIO_SND_PCM_FMT_U16 => AudioFormat::U16be,
            VIRTIO_SND_PCM_FMT_S18_3 => AudioFormat::S18le,
            VIRTIO_SND_PCM_FMT_U18_3 => AudioFormat::U18le,
            VIRTIO_SND_PCM_FMT_S20_3 => AudioFormat::S20le,
            VIRTIO_SND_PCM_FMT_U20_3 => AudioFormat::U20le,
...
mod tests {
    ...
    rusty_fork_test! {
        #[test]
        fn test_gstreamer_input_stream() {
            crate::init_logger();
            let stream = Stream {
    ...
    rusty_fork_test! {
        #[test]
        fn test_gstreamer_output_stream() {
            crate::init_logger();
            let stream = Stream {
    ...
    rusty_fork_test! {
        #[test]
        fn test_gstreamer_backend_as_any() {
            crate::init_logger();
            let stream_params = Arc::new(RwLock::new(vec![Stream::default()]));
            ...
            assert!(any_ref.downcast_ref::<GStreamerBackend>().is_some());
        }
    }

    rusty_fork_test! {
        #[test]
        fn test_gstreamer_mixed_input_output_streams() {
            crate::init_logger();
            ...
            let stream_params = Arc::new(RwLock::new(vec![input_stream, output_stream]));
            ...
            // Test both input and output streams
            for stream_id in 0..2 {
                gst_backend.set_parameters(stream_id, request).unwrap();
                gst_backend.prepare(stream_id).unwrap();
                gst_backend.release(stream_id).unwrap();
            }
        }
    }
