            ...
            gst_app::AppSinkCallbacks::builder()
                .new_sample(move |appsink| {
                    ...
                    let stream = match stream_params.get_mut(stream_id as usize) {
                        Some(s) => s,
                        None => {
                            log::error!("Stream {} not found in appsink callback", stream_id);
                            return Err(gst::FlowError::Error);
                        }
                    };
...
mod tests {
    ...
    use crate::{virtio_sound::*, stream::Stream};

    rusty_fork_test! {
        #[test]
        fn test_gstreamer_backend_initialization() {
            crate::init_logger();
            let stream_params = Arc::new(RwLock::new(vec![Stream::default()]));

            let _test_harness = GStreamerTestHarness::new();

            let gst_backend = GStreamerBackend::new(stream_params.clone());
            assert!(gst_backend.stream_out.read().unwrap().is_empty());
    ...
    rusty_fork_test! {
        ...
        fn test_gstreamer_invalid_state_transitions() {
            ...
            // Test that we can't stop before starting
            assert!(gst_backend.stop(0).is_err());
        }
    }
